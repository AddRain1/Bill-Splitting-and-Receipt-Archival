...
//         var ret = emailResults.map(result => new Users(result.user_id, result.username,  result.firstname, result.lastname, result.email, result.hashed_password, result.profile_description));
//     }
...
//     const ret = results.map(result => new Users(result.user_id, result.username, result.firstname, result.lastname, result.email, result.hashed_password, result.profile_description));
...
initPassportUsername(passport, async username => {
    ...
    if (userResults.length != 0){
        var ret = userResults.map(result => new Users(result.user_id, result.username, result.firstname, result.lastname, result.email, result.hashed_password, result.profile_description));
    }
...
app.post("/loginUsername", checkNotAuthenticated, passport.authenticate("local", {
    successRedirect: "/",
...
app.post("/signup", checkNotAuthenticated, async (req, res) => {
    try{
        ...
        const userQuery = 'INSERT INTO users (username, firstname, lastname, email, hashed_password, profile_description) VALUES (?, ?, ?, ?, ?, ?)';
        const userValues = [req.body.username, req.body.firstname, req.body.lastname, req.body.email, hash_password, req.body.profileDescription];
